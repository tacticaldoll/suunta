...
- [x] 1.1 Derive `PartialEq, Eq` on `SatisfactionFinding`, `Fix`,
      `CoverageFinding`, and `Sounding` in `crates/suunta-contract/src/lib.rs`.
- [x] 1.2 Derive `PartialEq, Eq` on `Correction<Body>`, `Course<Body>`,
      `Bearing<Body>`, and `Residual<Body>`.
- [x] 1.3 Add unit tests demonstrating equality for at least one body-free type
      and one `Body`-generic type (equal case and unequal case).
- [x] 1.4 Run the full Definition of Done (`AGENTS.md`) and confirm it passes.
